...
Reads the Our World in Data "CO2 emissions per capita" export from
``data/raw/co2_per_capita.csv`` and draws two ranked bar charts:

1. High-emitting microstates & oil producers
2. Major global economies & averages

Run from the project root::

    python notebooks/co2_bars.py

Writes both figures to ``reports/figures/``.
"""
...
def main() -> None:
    ...
    src = "Source: Our World in Data, CO₂ emissions per capita (2024). ▲▼ = change since 2014."

    fig1 = ranked_bar(
        subset(wide, OIL), category="Entity", value=f"y{LATEST}", compare=f"y{PRIOR}",
    ...
    fig2 = ranked_bar(
        subset(wide, ECON), category="Entity", value=f"y{LATEST}", compare=f"y{PRIOR}",
        vmax=vmax, unit="t",
