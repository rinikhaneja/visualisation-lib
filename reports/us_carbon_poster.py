...
from viz_lib import stacked_area, ranked_bar  # noqa: E402
...
def load_hero(raw=RAW) -> pd.DataFrame:
    df = pd.read_csv(raw / "us_co2_by_fuel.csv")
    for f in FUELS:
        df[f] = pd.to_numeric(df[f], errors="coerce") / 1e9
    return df


def load_share(raw=RAW) -> pd.DataFrame:
    files = {"Oil": "share_cumulative_oil.csv", "Coal": "share_cumulative_coal.csv",
...
def build_poster(data_dir=None):
    """Build and return the poster Figure. ``data_dir`` overrides where the
    CSVs are read from (e.g. ``data/demo`` in a cloned Colab checkout)."""
    raw = Path(data_dir) if data_dir else RAW
    apply_theme()
    ...
    stacked_area(load_hero(raw), x="Year", series=FUELS, ax=ax_hero,
    ...
    # supporting panels
    draw_ghg(fig.add_subplot(gs[1, 0]), raw)
    ranked_bar(load_share(raw), category="Fuel", value="share", value_fmt="{:.0f}%",
